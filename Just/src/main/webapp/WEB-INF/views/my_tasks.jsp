...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-9 col-md-10 main">
			<h1 class="page-header"><my:i18n name="str22"/></h1>
				...
				<tbody>
					<c:forEach var="task" items="${tasks}">
						<tr>
							...
							<td><c:out value="${task.tasks.date}" /></td>
							<td><c:out value="${task.tasks.description}" /></td>
							<td><select class="task-status form-control input-sm"
								data-task-id="${info4.id}">
									<c:forEach var="status" items="${statusesList}">
										<option value="${status.id}"
											<c:if test="${status.id==status.id}">selected</c:if>>${status.label}</option>
									</c:forEach>
							</select></td>
						</tr>
					</c:forEach>
